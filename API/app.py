...
@app.post("/search/")
async def search(query: Optional[str] = Form(default=None), top_k: Optional[int] = Form(100)) -> List[int]:
    if query:
        print(f"Received text query: {query}")
        text_embedding = mistral_embedding.embed_documents([query])[0]
        response = supabase.rpc("match_documents", {"query_embedding": text_embedding}).execute()
        ...
        raise HTTPException(status_code=400, detail="Query parameter 'query' is required for text queries.")


@app.post("/search/images/")
async def search_images(file: UploadFile = File(...), top_k: Optional[int] = Form(100)) -> List[int]:
    print(f"Received file: {file.filename}")
    contents = await file.read()
    image = Image.open(BytesIO(contents)).convert("RGB")  # Convert image to RGB
