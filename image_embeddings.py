...
def download_and_update_images():
    ...
    for record in response.data:  # Iterate over response.data instead of response["data"]
        image_id = record["id"]
        ...
        image_embeddings_list = image_embeddings.tolist()  # Convert ndarray to list
        
        # Update the `image_embeddings` column in the `posts` table
        update_query = supabase.table("posts").update({"image_embeddings": image_embeddings_list}).eq("id", image_id).execute()
